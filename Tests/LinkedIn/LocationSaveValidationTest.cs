...
public class LocationSaveValidationTest : LinkedInTestBase
{
    ...
    public async Task TC_V01_SaveConLocationVacio_RespetaObligatoriedad()
    {
        ...
        TestContext.Progress.WriteLine(
            $"[TC-V01] guardadoExitoso={_guardadoExitoso}, modalAbierto={modalAbierto}, hayError={hayError}");

        if (esRequerido)
        {
            // Requerido: no debe guardar con Location vacío.
            Assert.That(_guardadoExitoso, Is.False,
        ...
        {
            // No requerido: debe permitir guardar (modal cierra o no hay error bloqueante).
            Assert.That(_guardadoExitoso || !hayError, Is.True,
    ...
    [TearDown]
    public async Task RestaurarPerfilSiCambioAsync()
    {
        if (!_guardadoExitoso || string.IsNullOrWhiteSpace(_valorOriginal))
        {
            return;
        }

        TestContext.Progress.WriteLine(
            $"[TC-V01] Restaurando Location='{_valorOriginal}' tras Save que alteró el perfil.");
        ...
        try
        {
            ...
            var field = Page
                ...
                .First;
            await RestaurarLocationAsync(field, _valorOriginal);
        }
        catch (System.Exception ex)
        {
            TestContext.Progress.WriteLine($"[TC-V01] No se pudo restaurar Location: {ex.Message}");
        }
    }
}
